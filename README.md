...
> **Not yet run.** The ablation needs a migrated, seeded database and provider
> keys (Anthropic by default, or OpenAI alone via `LLM_PROVIDER=openai`). The
> golden set, metric definitions and runner are all in the repo; the false-flag
> rate — items sent to human review that were actually correct — is the
> headline metric, because it is the cost of over-caution Marcus would pay
> reviewing a tool that flags too much.

## Cost and latency per proposal

Design budget from the rate table (whisper $0.006/min, Haiku $1/$5 per MTok, Sonnet
$3/$15, embeddings $0.02/MTok); the ablation measures the real numbers. Chat tiers
default to Claude; `LLM_PROVIDER=openai` swaps them to gpt-4o-mini (classify) and
gpt-4o (extract/narrative) at OpenAI list prices.
...
6. `cp .env.example .env.local` — paste the local `supabase status` URL + service key;
   ...
   or set `LLM_PROVIDER=openai` to run chat on GPT-4o with the OpenAI key alone.
